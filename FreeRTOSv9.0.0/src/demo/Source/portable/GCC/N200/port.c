...
void vPortEnterCritical( void )
{
	//printf("vPortEnterCritical\n");
		...
		eclic_set_mth ((configMAX_SYSCALL_INTERRUPT_PRIORITY)<<(8-3));
	#endif
...
void vPortSetupTimer()	{
    ...
    uint64_t now = TMR_REG64(TMR_MTIME_OFFSET);
  	uint64_t then = now +  TMR_FREQ;// 
  	TMR_REG64(TMR_MTIMECMP_OFFSET) = then;


    mtime_intattr=eclic_get_intattr (ECLIC_IRQ_ID_MTIP);
  	mtime_intattr|=ECLIC_INT_ATTR_SHV;                          
  	eclic_set_intattr(ECLIC_IRQ_ID_MTIP,mtime_intattr);   //  set mtime_irq to  SHV MODE
